...
if torch_spec is None:
    raise ImportError(
        f"TorchPrimitiveSDF requires PyTorch >={TORCH_MIN_VER}, <={TORCH_MAX_VER}, "
    ...
    if (torch_ver < parse_version(TORCH_MIN_VER) or
        ...
        if IGNORE_TORCH_VER:
            warnings.warn(
                f'TorchPrimitiveSDF is compatible with PyTorch >={TORCH_MIN_VER}, <={TORCH_MAX_VER}, '
...
if not torch.cuda.is_available():
    if os.getenv('FORCE_CUDA', '0') == '1':
        ...
        logging.warning(
            "Torch did not find available GPUs on this system.\n"
            "TorchPrimitiveSDF will install only with CPU support and will have very limited features.\n"
...
PACKAGE_NAME = 'torchprimitivesdf'
...
def write_version_file():
    version_path = os.path.join(cwd, 'torchprimitivesdf', 'version.py')
...
def get_extensions():
    ...
    define_macros = []
    include_dirs = []
    sources = glob.glob('torchprimitivesdf/csrc/**/*.cpp', recursive=True)
    ...
    if torch.cuda.is_available() or os.getenv('FORCE_CUDA', '0') == '1':
        ...
        sources += glob.glob('torchprimitivesdf/csrc/**/*.cu', recursive=True)
